/**
 *
 * Objeto Literal Index
 * @author: Edy Segura, edy@segura.pro.br
 *
 */

var Index = {

	init: function() {
		Index.showLastModified();
	},
	
	
	showLastModified: function() {
		var span = document.getElementById('last-modified');
		span.innerHTML = Index.lastModified();
	},
	
	
	lastModified: function() {
		var x = new Date (document.lastModified);
		Modif = new Date(x.toGMTString());
		Year = Index.takeYear(Modif);
		Month = Modif.getMonth();
		Day = Modif.getDate();
		Mod = (Date.UTC(Year,Month,Day,0,0,0))/86400000;
		x = new Date();
		today = new Date(x.toGMTString());
		Year2 = Index.takeYear(today);
		Month2 = today.getMonth();
		Day2 = today.getDate();
		now = (Date.UTC(Year2,Month2,Day2,0,0,0))/86400000;
		daysago = now - Mod;
		if (daysago < 0) return '';
		unit = 'dias';
		if (daysago > 730) 	{
			daysago = Math.floor(daysago/365);
			unit = 'anos';
		}
		else if (daysago > 60) {
			daysago = Math.floor(daysago/30);
			unit = 'meses';
		}
		else if (daysago > 14) {
			daysago = Math.floor(daysago/7);
			unit = 'semanas'
		}
		var towrite = new String();
		if (daysago == 0) towrite += 'hoje';
		else if (daysago == 1) towrite += 'ontem';
		else towrite += daysago + ' ' + unit + ' ago';
		return towrite;
	},
	
	
	takeYear: function(theDate) {
		var x = theDate.getYear();
		var y = x % 100;
		y += (y < 38) ? 2000 : 1900;
		return y;
	}
	
};

//inicializacao
window.onload = Index.init;